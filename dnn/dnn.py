...
if __name__ == "__main__":
    ...
    #Compiling the RNN
    model.compile(optimizer = 'adam', loss = 'mean_squared_error', metrics=['accuracy', metrics.RootMeanSquaredError()])
    ...
    stock_prediction_rnn = np.reshape(stock_prediction_rnn, (stock_prediction_rnn.shape[0],))
    test_df_predictions = test_df
    test_df_predictions = test_df_predictions.drop(columns=['Target'])
    test_df_predictions['Target'] = stock_prediction_rnn.tolist()

    df_csv = test_df_predictions.to_csv('df_csv.csv')